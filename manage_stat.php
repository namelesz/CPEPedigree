...
<div class="container">
  <div class="row">
    <div class="col-md-4">
      ...
      <div class="panel panel-primary">
        <div class="panel-heading text-center">
          จำนวนวิชาที่อาจารย์แต่ละคนสอน
        </div>
        <table class="table" id="teacher-teaching-list">
    ...
    <div class="col-md-4">
      ...
      <div class="panel panel-primary">
        <div class="panel-heading text-center">
          จำนวนครั้งที่เลี้ยงสายของแต่ละสายรหัส
        </div>
        <table class="table" id="hanging-count-list">
      ...
      <div class="panel panel-primary">
        <div class="panel-heading text-center">
          จำนวนน้องเทคในแต่ละสายรหัส
        </div>
        <table class="table" id="taking-count-list">

        </table>
      </div>
      <div class="panel panel-primary">
        <div class="panel-heading text-center">
          จำนวนนักศึกษาในแต่ละสายรหัส
        </div>
        <table class="table" id="family-count-list">
    ...
    <div class="col-md-4">
      ...
      <div class="panel panel-primary">
        <div class="panel-heading text-center">
          <span class="glyphicon glyphicon-apple" aria-hidden="true"></span> จำนวนอาจารย์ที่สอนในแต่ละวิชา
        </div>
        <table class="table" id="teaching-list">
      ...
      <div class="panel panel-primary">
        <div class="panel-heading text-center">
          อายุเฉลี่ยของแต่ละรุ่น
        </div>
        <table class="table" id="age-list">
...
$(document).ready(function(){
  ...
  $.post("api/gen_age.php", {}, function(data){
    for(i=0;i<data.length;i++){
      $("#age-list").append("<tr><td class='text-center col-md-6'>" + data[i].generation + "</a> </td><td class='col-md-6'>อายุเฉลี่ย " + data[i].Age + " ปี</td></tr>")
    }
  })

  $.post("api/analysis_countCourse.php", {}, function(data){
    for(i=0;i<data.length;i++){
      $("#teaching-list").append("<tr><td class='text-center col-md-6'>" + data[i].courseID + "</a> </td><td class='col-md-6'>มีผู้สอน " + data[i].count + " คน</td></tr>")
    }
  })

  $.post("api/analysis_countHanging.php", {}, function(data){
    for(i=0;i<data.length;i++){
      $("#hanging-count-list").append("<tr><td class='text-center col-md-6'>สายรหัส " + data[i].familyCode + "</a> </td><td class='col-md-6'>" + data[i].count + " ครั้ง</td></tr>")
    }
  })

  $.post("api/taking_count.php", {}, function(data){
    for(i=0;i<data.length;i++){
      $("#taking-count-list").append("<tr><td class='text-center col-md-6'>" + data[i].StudentUpperID + "</a> </td><td class='col-md-6'>" + data[i].Count + "</td></tr>")
    }
  })

  $.post("api/familycode_count.php", {}, function(data){
    for(i=0;i<data.length;i++){
      $("#family-count-list").append("<tr><td class='text-center col-md-6'>สายรหัส " + data[i].FamilyCode + "</a> </td><td class='col-md-6'>จำนวน " + data[i].Count + "</td></tr>")
    }
  })


  $.post("api/teaching_count.php", {}, function(data){
    for(i=0;i<data.length;i++){
      $("#teacher-teaching-list").append("<tr><td class='text-center col-md-6'>" + data[i].TeacherFirstName + " " + data[i].TeacherLastName + "</a> </td><td class='col-md-6'>จำนวน " + data[i].Count + " วิชา</td></tr>")
    }
  })
})
</script>
